...
@pytest.fixture
def test_string():
    string = '''PCFET0NUWVBFIEhUTUwgUFVCTElDICItLy9XM0MvL0RURCBIVE1MIDQuMDEvL0VOIiAiaHR0cDov
...
@pytest.fixture
def emails_inst():
    filename = '1490137298.M907226P41655Q1.Pauls-MBA.cable.rcn.com'
    file = os.getcwd() + '\\Test_data\\mails\\'
    result = os.getcwd() + '\\Test_data\\results\\'
...
def test_decode(test_string, ret_inst):
    decoded_data = ret_inst.decode_data(test_string)
    string = ''.join([i for i in ret_inst.extract_from_html()])
    assert langdetect.detect(string) == 'en'


@pytest.raises(FileNotFoundError)
def test_result_location_saved():
    filename = '1490137303.M638828P41655Q17.Pauls-MBA.cable.rcn.com'
    file = open(os.getcwd() + '\\Test_data' + '\\results')


@pytest.raises(AssertionError)
def test_word_frequency(test_string, ret_inst):
    decoded_data = ret_inst.decode_data(test_string)
    get_all_lines = ''.join([i for i in ret_inst.extract_from_html(decoded_data)])
    freq = Count_freq(get_all_lines)
    test_dict = {'to': 6, 'your': 4, 'with': 3, 'you': 3, 'the': 2}
    false_detect = True
    for i in test_dict.keys():
        if test_dict[i] != freq[i]:
            false_detect = False
...
def test_for_language(test_string, ret_inst):
    from bs4 import BeautifulSoup
    file = open(os.getcwd() + 'Test_data\\' + '1490137298.M907226P41655Q1.Pauls-MBA.cable.rcn.com.html')
    soup = BeautifulSoup(file)
    lang = soup.find_all('p')
    a = lang.index('en')
    a = lang.index('8')
